...
class World:
                ...
                for j in range(len(self.color_list)):  # num of colors 8
                    for k in range(self.num_sequences_per_type): # 10
                        ...
                        while not shape_placed:
                            ...
                            position = self.choose_random_start_position(shape.dimensions)
    ...
    def choose_random_start_position(self, dimensions):
        if self.custom_bounds is not None:
            position = [random.randint(self.custom_bounds[0],
                                       min(self.num_columns-dimensions[0], self.custom_bounds[1])),
                        random.randint(self.custom_bounds[2],
                                       min(self.num_rows-dimensions[1], self.custom_bounds[3]))]
        else:
            position = [random.randint(0, self.num_columns-dimensions[0]),
                        random.randint(0, self.num_rows-dimensions[1])]
